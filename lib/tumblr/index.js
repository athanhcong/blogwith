...
app.createPostWithNote =  function(user, evernoteNote, callback) {
  ...
  contentInMarkdown(user, evernoteNote, function(error, noteContent) {
    ...
    return app.createPost(user, tumblrPost, function(error, tumblrPost) {
      // 
      console.log('Created Post: ' + evernoteNote.title + ' - Response: ' + JSON.stringify(tumblrPost));
      // Save to database
      if (error) {
        console.log(error);
        callback(error); 
        ...
        var post = {
          ...
          , 'evernote.note' : note
          , 'tumblr.post' : tumblrPost 
          , 'evernoteUpdated' : note.updated
          , 'updated' : new Date()
        };

        app.db.posts.update({evernoteGuid: note.guid}, {$set: post}, {upsert: true}, function(error) {
          if (error) console.log('ERROR: ' + error);
          callback (error, post);
        });
