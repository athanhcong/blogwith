...
var uploadResources = function(user, note, callback) {
  ...
  if (evernoteResources) {
    ...
    evernoteResources = [evernoteResources[0]]; // only 1 resource for now
  } else {
    return callback(null, []);
  };
  ...
  flow.serialForEach(evernoteResources, 
    ...
    , function(error, resource, tumblrPost) {
        ...
        callback(null, uploadedResources, tumblrPost);
    }
...
app.updatePostWithNote = function(user, post, evernoteNote, callback) {
  console.log('updatePostWithNote: ' + JSON.stringify(post.tumblr));

  var tumblrPostId = post.tumblr.post.id;
