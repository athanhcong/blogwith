...
var fileContentInMarkdown = function(userInfo, evernoteNote, resourceUrls) {
  var content = EvernoteLib.contentInMarkdown(userInfo, evernoteNote, resourceUrls);

  var title = note.title;
  contentMarkdown = '---\nlayout: post\ntitle: ' + title + '\n---\n' + contentMarkdown;

  return contentInMarkdown;
}
  ...
  noteStore.getResourceData(user.evernote.oauthAccessToken, evernoteResource.guid, 
    function onsuccess(fileData) {
      ...
      app.repoWithUser(user, function(err, repo) {
        ...
        app.createFileOrRetrieve(path, fileDataBase64, function(error, data) {
