...
app.filenameForJekyllPost = filenameForJekyllPost = function(user, note) {
  ...
  var titleFilename = safeFileUrl(title);
...
var safeFileName = function(string) {
  var safeString = string.replace(/[^A-Za-z0-9\. \-\_]*/g, '').trim();
  safeString = safeString.toLowerCase().split(' ').join('-');
  return safeString;
}

var safeFileUrl = function(string) {
...
var uploadAResource = function (user, note, evernoteResource, callback) {
  ...
  noteStore.getResourceData(user.evernote.oauthAccessToken, evernoteResource.guid, 
    function onsuccess(fileData) {
      ...
      console.log(fileDataBase64.length);

      app.repoWithUser(user, function(err, repo) {
        if (err) {
          console.log("Get Resource Repo error : " + err);
        ...
        var filename = evernoteResource.attributes.fileName;
        if (!filename) filename = evernoteResource.guid;
        var resourceFilename = safeFileName(filename);
    ...
    function onerror(error) {
      console.log("Get Resource onerror : " + error);
      callback(error);
    });
}
