...
var uploadResources = function(user, note, callback) {
  ...
  var callbackWithResources = function (resources) {
    ...
    callback(null, uploadedResourcesUrls, null);
  }
  ...
  flow.serialForEach(evernoteResources, 
    ...
    , function () {
      ...
      if (notYetUploadedEvernoteResources.length > 0) {
        uploadToWordpressResources(user, note, notYetUploadedEvernoteResources, true, 
...
var wpPostDraftWithMediaUrls = function(blog, mediaUrls, oauthAccessToken, callback) {
  ...
  form.append('content', 'Love to blog in Wordpress!');
...
var contentInMarkdown = function(user, evernoteNote, callback) {
  ...
  uploadResources(user, evernoteNote, function(error, resources, wordpressPost) {
    ...
    if (!error && resources) {
      ...
      callback(null, noteContent, wordpressPost);
    } else {
      callback (error);
    }
