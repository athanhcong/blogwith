...
var uploadToWordpressResources = function (user, note, evernoteResources, keepWordpressPost, callback) {
  ...
  wpPostDraftWithMediaUrls(user.wordpress.blog, user.wordpress.blog.ID, evernoteResourcesUrls, user.wordpress.oauthAccessToken, 
...
var uploadResources = function(user, wordpressPostId, note, callback) {
  ...
  var callbackWithResources = function (allResources, newWordpressPostId) {
    ...
    console.log('callbackWithResources - all resources: ' + allResources.length);
...
var wpPostDraftWithMediaUrls = function(wordpressBlog, blog, mediaUrls, oauthAccessToken, callback) {
  var oa = new OAuth2({
    clientId: config.wordpressClientId,
    clientSecret: config.wordpressClientSecret
  });
  ...
  function (error, response, body) {
    ...
    var callbackWithPost = function(AWordpressPost) {
      var wordpressResources = [];

      for(var key in AWordpressPost.attachments) {
        var wordpressResource = AWordpressPost.attachments[key];
        wordpressResources.push(wordpressResource);
      }

      console.log(wordpressResources);
      callback(null, wordpressResources, AWordpressPost);
    };


    if (wordpressBlog.jetpack) {
      var postURL = wordpressPost.meta.links.self;
      console.log('Jetpack site: postURL: ' + postURL);
      wpGet(postURL, oauthAccessToken, function(error, data) {
        if (error) {
          callback(error);
          return;
        };
        wordpressPost = JSON.parse(data);
        callbackWithPost(wordpressPost);  
      });
    } else {
      callbackWithPost(wordpressPost);
    }
  ...
  form.append('content', 'This draft post for uploading photo');
...
app.get('/wordpress/create', function(req, res) {
  ...
  //var path = '/sites/55523211/posts/new';
  var path = '/sites/55937282/posts/new';  


  wpPostMultiPath2(path, null, oauthAccessToken, function(error, data) {
    res.send(data, 200);
  });
...
app.get('/wordpress/delete', function(req, res) {
  ...
  wpDeletePostWithId(req.user, '218', function () {
...
app.updatePostWithNoteAndId = function(user, wordpressPostId, evernoteNote, callback) {
  ...
  contentInMarkdown(user, wordpressPostId, evernoteNote ,function(error, noteContent, newWordpressPostId) {
    ...
    if (!wordpressPostId) { // if not update post, I need to have add time.
      var createdDate = EvernoteLib.createdDateForNote(evernoteNote, 0);
      var createdDateString = moment(createdDate).zone(-1 * user.timezoneOffset).format("YYYY-MM-DDTHH:mm:ssZ");
      wordpressPostRequest.date = createdDateString;
    };
...
//  https://public-api.wordpress.com/rest/v1/sites/$site/posts/$post_ID/delete
  ...
  return wpPostPath(path, null, oauthAccessToken, function(error, wordpressPostString) {
    if (error && error.statusCode == 404) {
      console.log('Post is deleted: ' + wordpressPostId);
      callback(null);
      return;
    };


    var wordpressPost = JSON.parse(wordpressPostString);

    console.log(wordpressPost.status);
