...
function URLOfResource(guid, shardId, name){
...
var contentInMarkdown = function(userInfo, note) {
  // Create resource map
  var resourcesMap = [];
    if (note.resources) {
    for (var i = 0; i < note.resources.length; i++) {
      ...
      // Resource data
      resourceData = {
        url : URLOfResource(resource.guid, userInfo.shardId, resource.attributes.fileName)
      };
