...
function URLOfResource(guid, shardId, name, width){
  ...
  return 'https://'+ defaultServiceHost +'/shard/'+shardId+'/res/'+guid + '/' + escape(name) + '?resizeSmall&width=' + width;
}
...
var contentInMarkdown = function(userInfo, note, resourceUrls) {
  ...
  if (note.resources) {
    console.log('Resource count ' + note.resources.length);
    ...
    for (var i = 0; i < note.resources.length; i++) {
      ...
      if (resourceUrls[resource.guid]) {
        ...
        resourcesMap[bodyHashString] = resourceData;        
      } else {
...
app.createdDateForNote = createdDateForNote = function(note) {
